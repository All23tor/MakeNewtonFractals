...
class NewtonFractal{ // Builds a Newton Fractal pixel by pixel given a polynomial;
    ...
    Color printableColor(double x, double y){ // This checks which root each point is closest to, then assigns it a color
        ...
        for (int it=0;it<maxIteration;it++){
            ...
            for (int k=0;k<number_of_roots;k++){ //This is reaaaaally slow
                if (norm(z - roots[k])<tolerance){
                    return colors[k].scale((1.0+invmaxItEx)/(it*eccentricity+1)-invmaxItEx); // Scale the color such that it fades to black appropiately
                }   
            }
        }
    ...
    NewtonFractal(std::vector<std::complex<double>> _coefficients, int width = 1600, int height = 900, double _xr = 5.0): pixel_height{height}, pixel_width{width} { // Initializes everything
        ...
        if (isManual=='Y'){
            ...
            for (Color& color: colors){
                ...
                unsigned char r,g,b;
                std::cin >> hexColor;
                sscanf(hexColor.c_str(), "%02hhx%02hhx%02hhx", &r, &g, &b); // Read each color in hex format
                color = Color(r,g,b);
            }
        }else{
